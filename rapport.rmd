...
## Definition des options globales
knitr::opts_chunk$set(include=TRUE, echo=FALSE,comment = "",fig.align = 'center',fig.width=9, fig.height=6,message=FALSE)
#warning=FALSE
...
```{r,include=FALSE}
pred.rpart <- predict(object=tune$finalModel, test,type='prob')
...
## 3- Randomforest 
```{r,include=FALSE}
gridsearch1 <- expand.grid(mtry = seq(30,200,50))
tune1 <- train(subset(train, select=- c(outcome,outcome1)),ytrain1,method = "rf",tuneGrid=gridsearch1, trControl =objControl,metric='ROC')
```
...
```{r,include=FALSE}
# PRED
predxgb <- predict(tune3, subset(test, select=- c(outcome)))
predxgb.prob <- predict(tune3, subset(test, select=- c(outcome)), type="prob")
```
...
ERREUR_XGBOOST<-(conf.mat3[1,2]+conf.mat3[2,1])/nrow(test) 
```
...
```{r,include=FALSE}
# PRED
predneur <- predict(tune4, subset(test, select=- c(outcome)))
predneur.prob <- predict(tune4, subset(test, select=- c(outcome)), type="prob")
```
...
legend(0, 0.7, c('rpart','forest','elasticnet','xgboost','reseaux neurones'), 1:5)
```